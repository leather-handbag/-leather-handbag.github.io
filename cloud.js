...
export async function updateTrainingGameState(values = {}) { const { data, error } = await supabase.rpc("update_training_game_state", { selected_map_code: values.selected_map ?? null, selected_region_code: values.selected_region ?? null, audio_enabled_value: values.audio_enabled ?? null, effects_quality_value: values.effects_quality ?? null }); fail(error); return data || {}; }
export async function markTrainingUnlockSeen(eventId) { const { data, error } = await supabase.rpc("mark_unlock_event_seen", { event_id: eventId }); fail(error); return data || {}; }
export async function fetchGuardianChallenge(mapCode) { const { data, error } = await supabase.rpc("get_guardian_challenge", { map_code_value: mapCode }); fail(error); return data || {}; }
export async function rerollGuardianChallenge(mapCode) { const { data, error } = await supabase.rpc("reroll_guardian_challenge", { map_code_value: mapCode }); fail(error); return data || {}; }
